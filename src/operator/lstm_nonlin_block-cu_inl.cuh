...
static __global__ void _cuda_lstm_nonlin_block__forward(
	const RealType * const __restrict__ i2h_plus_h2h,
 ...
 * @param1 i2h_plus_h2h_grad [B x 4H]
...
static __global__ void _cuda_lstm_nonlin_block_backward(
	      RealType * const __restrict__ i2h_plus_h2h_grad,
...
private:
		   ...
		   const std::vector < TBlob > & out_data)
	{
		...
		Tensor < gpu, 2, DType > i2h_plus_h2h = in_data[int(EnumOpInputs::I2HPlusH2H)]
			.get < gpu, 2, DType > (cuda_stream);
		
		// infer the parameters from the cell input
		_param.batch_size = i2h_plus_h2h.shape_[0];
		_param.state_size = i2h_plus_h2h.shape_[1] / 4;
...
public:
			      ...
			      const std::vector < TBlob > & aux_args)
	{
		...
		std::size_t in_expected = 2, out_expected = 2;

		CHECK_EQ( in_data.size(),  in_expected); // i2h_plus_h2h, state_c
		...
		CHECK_EQ(i2h_plus_h2h.CheckContiguous(), true);
			      ...
			      const std::vector < TBlob > & aux_args)
	{
		...
		CHECK_NE(req[int(EnumOpInputs::I2HPlusH2H)], kAddTo) << 
			"AddTo is not supported for i2h_plus_h2h.";
		CHECK_NE(req[int(EnumOpInputs::StateC)], kAddTo) << "AddTo is not supported for state_c.";	

		Stream < gpu > * cuda_stream = ctx.get_stream < gpu > ();
	
		Tensor < gpu, 2, DType > i2h_plus_h2h_grad = in_grad[int(EnumOpInputs::I2HPlusH2H)]
			.get < gpu, 2, DType > (cuda_stream);
		...
		CHECK_EQ(i2h_plus_h2h_grad.CheckContiguous(), true);
	...
	const unsigned batch_size, const unsigned state_size)
{
	...
	RealType  input_gate = __cu_sigmoid(i2h_plus_h2h[batch_idx_x4H_plus_state_idx + 0 * state_size]);
	RealType forget_gate = __cu_sigmoid(i2h_plus_h2h[batch_idx_x4H_plus_state_idx + 1 * state_size]);
	RealType  input_actv =         tanh(i2h_plus_h2h[batch_idx_x4H_plus_state_idx + 2 * state_size]);
	RealType output_gate = __cu_sigmoid(i2h_plus_h2h[batch_idx_x4H_plus_state_idx + 3 * state_size]);
	...
	const unsigned batch_size, const unsigned state_size)
{
	...
	i2h_plus_h2h_grad[batch_idx_x4H_plus_state_idx + 0 * state_size] =  input_gate_grad *  input_gate * (1 -  input_gate);
	i2h_plus_h2h_grad[batch_idx_x4H_plus_state_idx + 1 * state_size] = forget_gate_grad * forget_gate * (1 - forget_gate);
	i2h_plus_h2h_grad[batch_idx_x4H_plus_state_idx + 2 * state_size] =  input_actv_grad * (1 - input_actv * input_actv);
	i2h_plus_h2h_grad[batch_idx_x4H_plus_state_idx + 3 * state_size] = output_gate_grad * output_gate * (1 - output_gate);
}
