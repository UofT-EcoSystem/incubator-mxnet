...
inline NDArray ReshapeOrCreate(const std::string& name,
  ...
  if (it != shared_buffer->end()) {
    ...
    if (size_shareable && stype_shareable) {  // memory can be reused
      ...
      it->second = InitZeros(dest_arg_stype, dest_arg_shape, ctx, dest_arg_dtype, ndarray_name);
      return it->second;
    } else {
      // not shareable storage
      return InitZeros(dest_arg_stype, dest_arg_shape, ctx, dest_arg_dtype, ndarray_name);
    }
  } else {
    auto ret = InitZeros(dest_arg_stype, dest_arg_shape, ctx, dest_arg_dtype, ndarray_name);
    if (stype_shareable) {
